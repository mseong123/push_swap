...
R=$(leaks -atExit -- ./push_swap $ARG > GEMARTINTEMP && echo $?)
if [[ $R == 0 ]]; then
...
R=$(./push_swap $ARG | leaks -atExit -- ./checker $ARG > GEMARTINTEMP && echo $?)
if [[ $R == 0 ]]; then
  printf "${GREEN}[MOK] ${DEF_COLOR}";
else
  printf "${RED} [KO LEAKS] ${DEF_COLOR}";
